...
public class Bidder implements Client {
	...
	public Bidder(AuctionServer server, String name, int cash, int cycles,
			int maxSleepTimeMs, long randomSeed) {
		this.name = name;
		this.cash = cash;
		this.cycles = cycles;
		...
		for (int i = 0; (i < cycles && cash > 0) || activeBids.size() > 0; ++i) {
			...
			while (items.size() > 0) {
				...
				if (price < this.cash - sumActiveBids) {
					...
					boolean success = server.submitBid(this.name(),
							item.listingID(), price + 1);
