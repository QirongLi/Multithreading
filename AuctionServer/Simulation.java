...
public class Simulation {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < nrSellers; ++i) {
			sellers[i] = new Seller(AuctionServer.getInstance(), "Seller" + i,
					100, 50, i);
		...
		for (int i = 0; i < nrBidders; ++i) {
			bidders[i] = new Bidder(AuctionServer.getInstance(), "Buyer" + i,
					1000, 20, 150, i);
...
//		int count = server.getItemsAndIDs().size()-server.getItems().size();
