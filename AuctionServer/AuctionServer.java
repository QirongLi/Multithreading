...
public class AuctionServer {
	...
	public synchronized int submitItem(String sellerName, String itemName,
		...
		// Pre-condition: there's room in the item list, and there's room for
		// the seller to add new item
		// Post-condition: A new item has been added to item list.
		// Invariant: The item cannot be null.
		...
		Item item = new Item(sellerName, itemName, listingID,
		...
		// System.out.println(sellerName + "/" + itemName + "/" +
		// lowestBiddingPrice);
	...
	public synchronized List<Item> getItems() {
		...
		// Pre-condition:There is a item list in the auction site.
		// Post-condition: return a copy of item list to client.
		// Invariant: Client cannot change item list without auction server.

		List<Item> itemListCopy = new ArrayList<Item>(itemsUpForBidding);
	...
	public synchronized boolean submitBid(String bidderName, int listingID,
		...
		// Pre-condition: the item exists and can be bid upon, and the bidder
		// can bid for the item.
		// Post-condition: the item has a new price is the biddingAmount is
		// higher than the previous.
		// Invariant: the price for the item should be highest in all
		// biddingAmount.
		...
		if (itemsUpForBidding.contains(itemsAndIDs.get(listingID))
				&& itemsAndIDs.containsKey(listingID)) {
			if (itemsPerBuyer.get(bidderName) < maxBidCount) {
				...
				if (highestBids.containsKey(listingID)
					...
					itemsPerBuyer.put(formerBidder,
							itemsPerBuyer.get(formerBidder) - 1);
				}
				highestBidders.put(listingID, bidderName);
				highestBids.put(listingID, biddingAmount);
				if (itemsPerBuyer.containsKey(bidderName))
					itemsPerBuyer.put(bidderName,
							itemsPerBuyer.get(bidderName) + 1);
				else
	...
	public synchronized int checkBidStatus(String bidderName, int listingID) {
		...
		// Post-condition: item's status has been returned.
		// Invariant: item has only one status.

		if (itemsAndIDs.containsKey(listingID)) {
			Item item = itemsAndIDs.get(listingID);

			if (!item.biddingOpen()
					&& highestBidders.get(listingID) == bidderName) {

				itemsUpForBidding.remove(item);
				if (itemsPerBuyer.get(bidderName) > 0) {
					int value = itemsPerBuyer.get(bidderName) - 1;
					itemsPerBuyer.put(bidderName, value);
				}
				int itemNumber = itemsPerSeller.get(item.seller());
				itemsPerSeller.put(item.seller(), itemNumber - 1);
	...
	public synchronized int itemPrice(int listingID) {
		...
		// Pre-condition: item exists.
		// Post-condition: return item's price.
		// Invariant: every item has a price and the price should be positive.
		// Exception: if item does not exist, return -1;
		if (highestBids.containsKey(listingID))
			return highestBids.get(listingID);

		if (itemsAndIDs.containsKey(listingID)
				&& !highestBids.containsKey(listingID))
	...
	public Boolean itemUnbid(int listingID) {
		...
		// Pre-condition:item exsits and item has been bid already.
		// Post-condition:
		// Invariant: item in the list has a status, whether it has been bid
		// upon or not.
		if (!itemsAndIDs.containsKey(listingID))
			return true;
